...
export function PricingCard({ name, price, description, features, highlighted, mostPopularText = 'Most Popular' }: Props) {
  return (
    <div className={cn(
      ...
      highlighted ? 'border-accent bg-white shadow-subtle' : 'border-slate-200 bg-white'
    )}>
      <div className="mb-4 flex items-baseline justify-between">
        <h3 className="text-lg font-semibold text-primary">{name}</h3>
        {highlighted && (
          <span className="rounded-full bg-accent/10 px-2 py-1 text-xs font-semibold text-accent">{mostPopularText}</span>
        )}
      </div>
      <div className="mb-2 text-2xl font-extrabold text-primary">{price}</div>
